...
/**
 * Reliability/consistency harness for the Renode SPI TCP bridge and the firmware-managed slave.
 *
 * It connects to the Renode SPI TCP bridge and, for many iterations, sends a random payload to the
 * Renode SPI controller (which clocks it to the firmware-managed SPI slave), then polls for and reads
 * back the slave's response, checking it byte-for-byte. At the end it prints reliability statistics.
 *
 * Usage: java spi.Main [host] [port] [iterations] [payloadSize] [timeoutMs]
 */
public final class Main {

    public static void main(String[] args) throws Exception {
        String host = args.length > 0 ? args[0] : "127.0.0.1";
        int port = args.length > 1 ? Integer.parseInt(args[1]) : 33670;
        ...
        System.out.printf("Connecting to Renode SPI bridge at %s:%d%n", host, port);
        try (SPIBridge bridge = new SPIBridge(host, port)) {
    ...
    private static byte[] receiveExactly(SPIBridge bridge, int count, long timeoutMs) throws Exception {
