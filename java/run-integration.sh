#!/usr/bin/env bash
#
# End-to-end reliability run: starts a headless Renode with the firmware-managed I3C slave and the
# forward-on-interrupt TCP bridge, then runs the Java harness which drives the whole chain
# (Java -> TCP bridge -> I3C master -> firmware slave -> back) and reports reliability/consistency.
#
...
REPL="$RENODE_DIR/tests/peripherals/I3C-firmware.repl"
ELF="$RENODE_DIR/tests/peripherals/i3c-firmware.elf"
[ -f "$REPL" ] || REPL="$REPO/renode-overlay/tests/peripherals/I3C-firmware.repl"
[ -f "$ELF" ] || ELF="$REPO/firmware/i3c-firmware.elf"
...
sysbus LoadELF @$ELF
emulation CreateI3CTCPBridge sysbus.i3c 0x08 $PORT true
start
EOF
...
RENODE_PID=$!
# Hold the control FIFO open so Renode's monitor stays alive while we test.
exec 3>"$FIFO"
cleanup() {
...
java -cp "$HERE/out" i3c.Main 127.0.0.1 "$PORT" "$ITER" "$SIZE" "$TIMEOUT_MS"
